...
{
    public static class GremlinVisualFactory
    {
        const string ResourcePath = "Models/Creatures/gremlin";
        const float TargetBodyHeight = WorldScale.CharacterHeightUnits * 0.31f;
        ...
        public static GameObject CreateMonster(Vector3 position, float scaleMultiplier, string displayName)
        {
            ...
            if (prefab == null)
            {
                Debug.LogWarning($"Monster Miner: gremlin prefab not found at Resources/{ResourcePath}.");
                return null;
            }

            var gremlin = Object.Instantiate(prefab);
            gremlin.name = displayName;
            gremlin.transform.position = position;
            gremlin.transform.rotation = Quaternion.Euler(0f, Random.Range(0f, 360f), 0f);

            float prefabHeight = EstimatePrefabHeight(gremlin);
            float scale = prefabHeight > 0.01f
                ? TargetBodyHeight / prefabHeight * scaleMultiplier
                : scaleMultiplier;
            gremlin.transform.localScale = Vector3.one * scale;

            PrepareForGameplay(gremlin);
            return gremlin;
        }
        ...
        static void PrepareForGameplay(GameObject root)
        {
            ...
            if (root.GetComponent<GremlinLocomotion>() == null)
                root.AddComponent<GremlinLocomotion>();
        ...
        static void EnsureCapsuleCollider(GameObject root)
        {
            ...
            float radius = Mathf.Max(0.14f, Mathf.Max(Mathf.Abs(localSize.x), Mathf.Abs(localSize.z)) * 0.2f);
