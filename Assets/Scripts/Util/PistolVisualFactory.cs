...
{
    public static class PistolVisualFactory
    {
        const string ModelResourcePath = "Models/Tools/m1911";
        const float TargetHeldLength = 0.42f;

        static readonly Vector3 HeldLocalPosition = new Vector3(0.01f, -0.04f, 0f);
        static readonly Quaternion HeldLocalRotation =
            Quaternion.Euler(0f, -90f, 0f) * Quaternion.Euler(0f, 0f, 90f) * Quaternion.Euler(180f, 0f, 0f) * Quaternion.Euler(-15f, 0f, 0f);
        ...
        public static GameObject CreateHeldPistol(Transform parent, ItemDefinition item = null)
        {
            var model = Resources.Load<GameObject>(ModelResourcePath);
            if (model == null)
            {
                Debug.LogWarning($"Monster Miner: M1911 model not found at Resources/{ModelResourcePath}.");
                return null;
            }

            var pistol = Object.Instantiate(model, parent, false);
            pistol.name = "HeldPistol";
            HideEditorNodes(pistol);
            pistol.transform.localRotation = HeldLocalRotation;
            pistol.transform.localScale = ComputeHeldScale(pistol);
            pistol.transform.localPosition = HeldLocalPosition;
            ApplyUrpMaterials(pistol);
            KnifeVisualFactory.ApplyLegendaryGoldMaterialsIfNeeded(pistol, item);
            DisableColliders(pistol);
            return pistol;
        }

        static void HideEditorNodes(GameObject root)
        {
            foreach (var transform in root.GetComponentsInChildren<Transform>(true))
            {
                if (transform.name == "CINEMA_4D_Editor")
