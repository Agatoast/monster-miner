...
{
    public static class RifleVisualFactory
    {
        const string ModelResourcePath = "Models/Tools/m107";
        const float TargetHeldLength = 1.05f;
        ...
        public static GameObject CreateHeldRifle(Transform parent, ItemDefinition item = null)
        {
            ...
            if (model == null)
            {
                Debug.LogWarning($"Monster Miner: M107 model not found at Resources/{ModelResourcePath}.");
                return null;
            }

            var rifle = Object.Instantiate(model, parent, false);
            rifle.name = "HeldRifle";
            rifle.transform.localRotation = HeldLocalRotation;
