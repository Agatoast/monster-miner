...
{
    public static class MaraVisualFactory
    {
        const string ResourcePath = "Models/Creatures/mara";
        const float TargetBodyHeight = WorldScale.CharacterHeightUnits * 1.05f;
        ...
        public static GameObject CreateMonster(Vector3 position, float scaleMultiplier, string displayName)
        {
            ...
            if (prefab == null)
            {
                Debug.LogWarning($"Monster Miner: Mara horse prefab not found at Resources/{ResourcePath}.");
                return null;
            }

            var mara = Object.Instantiate(prefab);
            mara.name = displayName;
            mara.transform.position = position;
            mara.transform.rotation = Quaternion.Euler(0f, Random.Range(0f, 360f), 0f);

            float prefabHeight = EstimatePrefabHeight(mara);
            float scale = prefabHeight > 0.01f
                ? TargetBodyHeight / prefabHeight * scaleMultiplier
                : scaleMultiplier;
            mara.transform.localScale = Vector3.one * scale;

            PrepareForGameplay(mara);
            return mara;
        }
        ...
        static void EnsureCapsuleCollider(GameObject root)
        {
            ...
            float radius = Mathf.Max(0.12f, Mathf.Max(Mathf.Abs(localSize.x), Mathf.Abs(localSize.z)) * 0.18f);
