...
{
    public static class ShotgunVisualFactory
    {
        const string ModelResourcePath = "Models/Tools/bennelli_m4";
        const float TargetHeldLength = 0.88f;
        ...
        public static GameObject CreateHeldShotgun(Transform parent, ItemDefinition item = null)
        {
            var model = Resources.Load<GameObject>(ModelResourcePath);
            if (model == null)
            {
                Debug.LogWarning($"Monster Miner: Benelli M4 model not found at Resources/{ModelResourcePath}.");
                return null;
            }

            var shotgun = Object.Instantiate(model, parent, false);
            shotgun.name = "HeldShotgun";
            shotgun.transform.localRotation = HeldLocalRotation;
            shotgun.transform.localScale = ComputeHeldScale(shotgun);
            shotgun.transform.localPosition = HeldLocalPosition;
            ApplyUrpMaterials(shotgun);
            KnifeVisualFactory.ApplyLegendaryGoldMaterialsIfNeeded(shotgun, item);
            DisableColliders(shotgun);
            return shotgun;
        }
