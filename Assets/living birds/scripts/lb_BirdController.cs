...
public class lb_BirdController : MonoBehaviour {
	...
	Vector3 FindPointInGroundTarget(GameObject target){
		...
		point.x = Random.Range (targetCollider.bounds.max.x,targetCollider.bounds.min.x);
		point.y = targetCollider.bounds.max.y;
		point.z = Random.Range (targetCollider.bounds.max.z,targetCollider.bounds.min.z);
		//raycast down until it hits the ground
		RaycastHit hit;
		if (Physics.Raycast (point,-Vector3.up,out hit,targetCollider.bounds.size.y,groundLayer)){
			return hit.point;
		}
	...
	void BirdFindTarget(GameObject bird){
		...
		if (birdGroundTargets.Count > 0 || birdPerchTargets.Count > 0){
			...
			for (int i=0;i<birdGroundTargets.Count;i++){
				var groundCollider = birdGroundTargets[i].GetComponent<Collider>();
				if (groundCollider == null)
					continue;
				gtArea += groundCollider.bounds.size.x*groundCollider.bounds.size.z;
			}
