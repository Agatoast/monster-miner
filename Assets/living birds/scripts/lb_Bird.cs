...
public class lb_Bird : MonoBehaviour {
	...
	void PauseBird(){
		if(!dead){
			...
			anim.speed = 0;
			if(!birdRigidbody.isKinematic){originalVelocity = birdRigidbody.linearVelocity;}
	...
	void UnPauseBird(){
		if(!dead){
			...
			birdRigidbody.linearVelocity = originalVelocity;
			paused = false;
		}
	}
	...
	IEnumerator FlyToTarget(Vector3 target){
		if(Random.value < .5){
			birdAudio.PlayOneShot (flyAway1,.1f);
		}else{
			birdAudio.PlayOneShot (flyAway2,.1f);
		}
		flying = true;
		landing = false;
		onGround = false;
		...
		birdRigidbody.linearDamping = 0.5f;
		...
		birdRigidbody.AddForce((transform.forward * 50.0f*controller.birdScale)+(transform.up * 100.0f*controller.birdScale));
		float t = 0.0f;
		while (t<1.0f){
			if(!paused){
				t+= Time.deltaTime;
		...
		if(vectorDirectionToTarget.y>.5f){
			...
			while(vectorDirectionToTarget.y>.5f){
				...
				birdRigidbody.AddForce(transform.forward * 70.0f*controller.birdScale * Time.deltaTime);
				...
				if (Physics.Raycast(transform.position,-Vector3.up,out hit,0.15f*controller.birdScale) && birdRigidbody.linearVelocity.y < 0){
					//if the bird is going to collide with the ground zero out vertical velocity
					if(!hit.collider.isTrigger){
						birdRigidbody.linearVelocity = new Vector3(birdRigidbody.linearVelocity.x, 0.0f,birdRigidbody.linearVelocity.z);
					}
				}
				if (Physics.Raycast(transform.position,Vector3.up,out hit,0.15f*controller.birdScale) && birdRigidbody.linearVelocity.y > 0){
				...
				//check for collisions with non trigger primaryColliders and abort flight if necessary
		...
		while(true){
			if(!paused){
				...
				if (Physics.Raycast(transform.position,-Vector3.up,0.15f*controller.birdScale) && birdRigidbody.linearVelocity.y < 0){
					birdRigidbody.linearVelocity = new Vector3(birdRigidbody.linearVelocity.x, 0.0f,birdRigidbody.linearVelocity.z);
				}
				if (Physics.Raycast(transform.position,Vector3.up,out hit,0.15f*controller.birdScale) && birdRigidbody.linearVelocity.y > 0){
					//if the bird is going to collide with something overhead zero out vertical velocity
				...
				birdRigidbody.AddForce(transform.forward * flyingForce * Time.deltaTime);
				distanceToTarget = Vector3.Distance (transform.position,target);
		...
		birdRigidbody.linearDamping = .5f;
		birdRigidbody.linearVelocity = Vector3.zero;
		anim.SetBool(landingBoolHash, false);
		landing = false;
		transform.localEulerAngles = new Vector3(0.0f,transform.localEulerAngles.y,0.0f);
	...
	void PlaySong(){
		if (!dead){
			if(Random.value < .5){
				birdAudio.PlayOneShot (song1,1);
			}else{
				birdAudio.PlayOneShot (song2,1);
			}
		}
	}
